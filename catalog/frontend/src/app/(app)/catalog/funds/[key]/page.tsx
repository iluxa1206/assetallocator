      ...
      <div className="flex items-center gap-1 text-[11px] font-semibold uppercase tracking-[0.08em] text-muted-foreground">
        ...
        {info && (
          <span
            title={info}
            aria-label={info}
            className="inline-flex h-4 w-4 cursor-help items-center justify-center rounded-full border border-current text-[11px] leading-none opacity-80 hover:opacity-100"
          >
            ?
          </span>
        )}
...
function RiskBar({ score }: { score: number | null }) {
  ...
  return (
    <div className="flex items-center gap-2">
      <span className="shrink-0 text-[11px] uppercase tracking-[0.12em] text-muted-foreground">низкий</span>
      ...
      <span className="shrink-0 text-[11px] uppercase tracking-[0.12em] text-muted-foreground">высокий</span>
    </div>
  );
}
...
export default function FundDetailPage({ params }: { params: Promise<{ key: string }> }) {
  ...
  if (isError || !fund) return <div role="alert" className="text-destructive text-sm">Не удалось загрузить фонд</div>;
              ...
              value={
                fund.aum != null ? (
                  <span>
                    ...
                    {fund.aum_as_of && (
                      <span className="ml-1 text-[11px] font-normal text-muted-foreground">на {fmtDate(fund.aum_as_of)}</span>
                    )}
                  </span>
                ) : null
              }
                ...
                {fund.risks.map((r, i) => (
                  <div key={i} className="glossy rounded-xl p-3.5 text-sm">
                    <div className="flex items-center gap-2 mb-1.5">
                      ...
                      <span
                        className={cn(
                          "text-[11px] uppercase tracking-wider px-2 py-0.5 rounded font-semibold",
